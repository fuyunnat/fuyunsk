...
`references/context-memory-continuity.md` 规定长任务使用已被忽略的 `work/task-state.md` 或项目外等价位置，记录目标、决策、改动、验证、远端状态和回滚方式。状态文件只保存恢复所需摘要和证据路径，不记录密钥、隐私数据、生产凭证或完整敏感日志，也不进入业务提交。

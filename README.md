...
- 代码组织先看职责边界：不同功能、接口请求、业务规则、数据库访问、状态管理和 UI 组件不能为了省事堆在一个文件里。
...
`references/task-lanes.md` 是 read-only、quick、standard、full 及附加通道的唯一选择依据。默认先判断能否轻量完成：错别字、README、单页局部样式、按钮/标签/字体整理、明确的单文件小修，通常只需要目标文件、最近上下文和一个局部验证。完整规范继续保留全部详细规则，但不能反向扩大授权或把普通跨文件修改、任务分支上传、以及“稳一点”这类口语机械升级为完整通道。

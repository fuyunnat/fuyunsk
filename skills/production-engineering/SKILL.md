...
- Keep responsibility boundaries explicit. Before coding, decide where page/component, API client, state/store, business/service/domain, persistence/repository/DAO/model/migration, config, utilities, and tests belong in the real project. Do not add unrelated features to a file just because it is open; split by existing structure unless one cohesive flow is clearer, and explain that choice.

...
用户要求保留原貌或行为时，验收依据是迁移前源码及其可复现结果，不是新实现自己的截图。先读取关联 HTML/模板、CSS、资源、脚本与断点，保留明确的版本/路径；没有浏览器能力可继续已明确且可从源码确认的适配，但必须报告视觉一致性未验证，不能据此换皮肤或承诺像素一致。

按本次范围建立旧页面到新组件、旧交互到新状态的对应关系，优先复用原样式与素材。先完成一个代表性页面的对照再批量推进，局部任务只检查相关区域；不让一个示例升级成所有任务的全站迁移流程。相同浏览器、视口、字体、主题和固定数据下对比旧版、新版及差异，覆盖受影响的桌面/移动、弹窗、展开、加载、空和错误状态，并分别检查功能行为。

发现未授权差异先修新实现；不得用新截图覆盖旧基准、放宽容差、屏蔽区域或弱化断言来制造通过。动态区域确需归一化或容差时说明依据与受影响范围，不掩盖真实布局变化。只有源码静态检查、构建或单张新截图不算原样验收。原资源丢失或版本歧义查证后仍无法消除时反问，不要求用户重复提供可访问源码。

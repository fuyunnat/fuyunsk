...
- 用户明确授权吸收外部方法时，只提炼并重写适用流程，注明来源和许可证；不得复制整套第三方 skill、增加运行时依赖，或重新扩大常驻提示词。
- 外部方法必须作为按需 reference 接入；不得借此开启隐式调用、恢复长全局 `AGENTS.md`、或让普通对话加载工程流程。
...
如果自检脚本、路径、触发词、删除策略、Git 规则、上下文状态规则、数据库兼容演进、外部方法来源或前端后台默认栈有变化，必须说明影响范围和回滚方式。
